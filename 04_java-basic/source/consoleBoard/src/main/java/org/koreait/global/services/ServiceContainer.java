...
public class ServiceContainer {
     ...
     * 동적 객체 생성
     *
     * 객체를 동적으로 생성해서 objects 컨테이너에 담아 준다.
     * 필요할 때 Class 클래스 정보를 가지고 조회.
     */
    public void init() {
        for (Class<?> clazz : classes) {

            // @Configuration 애노테이션이 정의된 클래스: 생성할 객체를 설정
            if (clazz.getAnnotation(Configuration.class) == null) continue;

            try { // 기본 생성자가 없을 수도 있으니 예외 처리
                Object obj = clazz.getDeclaredConstructor().newInstance(); // 동적 객체 생성
                Method[] methods = obj.getClass().getDeclaredMethods();
                for (Method method : methods) {
                    // @Bean이 적용된 메서드만 동적 호출하여 객체 생성
                    if (method.getAnnotation(Bean.class) == null) continue;

                    // 동적 메서드를 호출해서 컨테이너가 관리할 객체 생성
