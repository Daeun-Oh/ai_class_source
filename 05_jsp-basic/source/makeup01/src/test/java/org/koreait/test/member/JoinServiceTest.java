...
public class JoinServiceTest {
    ...
    @Tag("success")
    @Test
    @DisplayName("회원가입을 성공한 경우, 예외가 발생하지 않는다.")
    void successTest() {
        assertDoesNotThrow(() -> {
    ...
    void requiredFieldValidationTest() {
        Map<String, String> messages = Map.of("email", "이메일",
                            "password", "비밀번호",
                            "confirmPassword", "비밀번호를 확인",
                            "name", "이름");
    ...
    @Test
    @DisplayName("비밀번호와 그 확인이 일치하지 않으면, BadRequestException 발생")
